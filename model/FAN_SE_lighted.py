...
def conv3x3(in_planes:int, out_planes:int, stride=1, padding=1, bias=False, dilation=1):
    "3x3 convolution with padding"
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, dilation=dilation,
...
class Depthwise_ConvBlock(nn.Module):
    def __init__(self, in_planes:int, out_planes:int):
        ...
        self.conv1 = nn.Conv2d(in_planes, int(out_planes / 2), kernel_size=1,stride=1, padding=0)
        self.bn1 = nn.BatchNorm2d(int(out_planes / 2))

        self.conv2 = depthwise_conv3x3(planes=int(out_planes / 2))
        self.bn2 = nn.BatchNorm2d(int(out_planes / 2))

        self.conv3 = depthwise_conv3x3(planes=int(out_planes / 2), dilation=2, padding=2)
        self.bn3 = nn.BatchNorm2d(int(out_planes / 2))

        self.conv4 = depthwise_conv3x3(planes=int(out_planes / 2), dilation=3, padding=3)
        self.bn4 = nn.BatchNorm2d(int(out_planes / 2))

        self.conv5 = conv1x1(int(out_planes / 2), out_planes)
    ...
    def forward(self, x):
        ...
        out1 = self.conv2(out)
        out1 = self.relu(self.bn2(out1))
        
        out2 = self.conv3(out)
        out2 = self.relu(self.bn3(out2))

        out3 = self.conv4(out)
        out3 = self.relu(self.bn4(out3))
...
class HourGlassNet(nn.Module):
    def __init__(self, depth:int, num_feats:int):
        ...
        for level in range(1, depth + 1):
            # upper branch
            self.add_module(f"shortcut{level}", Depthwise_ConvBlock(self.num_feats, self.num_feats))
            # lower branch
